...
public class FavVidNowPlayActivity extends Activity {
	...
	public VideoView vv_favvidnowplay;
	public boolean videoplay = true;
	public int favstatus = 0;
	public TextView tv_favvidnowplayfav, et_username;
	public ImageView iv_favvidnowplayfav;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.favvidnowplay_ui);
		...
		tv_favvidnowplayfav = (TextView) findViewById(R.id.tvfavvidnowplayfav);
		iv_favvidnowplayfav = (ImageView) findViewById(R.id.ivfavvidnowplayfav);
		vv_favvidnowplay = (VideoView) findViewById(R.id.vvfavvidnowplay);
		...
		if (favstatus == 1) {
			iv_favvidnowplayfav.setImageDrawable(getResources().getDrawable(
					R.drawable.faviconred));
			tv_favvidnowplayfav.setText("Favourite");
		} else if (favstatus == 0) {
			iv_favvidnowplayfav.setImageDrawable(getResources().getDrawable(
					R.drawable.playlisticon1));
			tv_favvidnowplayfav.setText("Pre-Mixed");
		}

		vv_favvidnowplay.setOnTouchListener(new OnTouchListener() {
			@Override
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					if (ll_menulayout.getVisibility() == View.VISIBLE) {
						...
						vv_favvidnowplay.start();
					} else {
						mediacontroller.show();
						if (videoplay) {
							vv_favvidnowplay.pause();
							videoplay = false;
		...
		try {
			...
			mediacontroller = new MediaController(
					FavVidNowPlayActivity.this);
			mediacontroller.setAnchorView(vv_favvidnowplay);
			// Get the URL from String VideoURL
			Uri video = Uri.parse(filename);
			vv_favvidnowplay.setMediaController(mediacontroller);
			vv_favvidnowplay.setVideoURI(video);
		...
		vv_favvidnowplay.requestFocus();
		vv_favvidnowplay.setOnPreparedListener(new OnPreparedListener() {
	...
	public void clickhome(View v) {
		if (ll_menulayout.getVisibility() == View.VISIBLE) {
			...
			Toast.makeText(getApplicationContext(), "Home", Toast.LENGTH_LONG)
					.show();
			finish();
			Intent homeintent = new Intent(FavVidNowPlayActivity.this,
					HomeActivity.class);
